...
def sentence_split(sentence, word_dict, w2i):
...
def process_captions(label_file, word_dict, w2i):
    ...
    for d in label:
        for s in d['caption']:
            s = sentence_split(s, word_dict, w2i)
...
class training_data(Dataset):
    def __init__(self, label_file, files_dir, word_dict, w2i):
        ...
        self.w2i = w2i
        self.data_pair = process_captions(files_dir, word_dict, w2i)
...
# Encoder class
class EncoderRNNLayer(nn.Module):
    def __init__(self):
        super(EncoderRNNLayer, self).__init__()
...
def main():
    ...
    encoder = EncoderRNNLayer()
    decoder = DecoderRNNLayer(512, len(i2w) + 4, len(i2w) + 4, 1024, dropout_percentage=0.5)
    ...
    torch.save(model, "{}/{}.h5".format('SavedModel', 'modelsowmya'))
